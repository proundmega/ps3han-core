...
def pack(folder, contentid, outname=None, gameType=0xE):
	...
	metaBlock.unk31 	= gameType   #packageType 0x10 == patch, 0x8 == Demo&Key, 0x0 == Demo&Key (AND UserFiles = NotOverWrite), 0xE == normal, use 0xE for gamexec, and 8 for gamedata
	...
	for file in files:
		if not file.flags & 0xFF == TYPE_DIRECTORY:
			# small fix due to this script doesn't work if using absolute paths
			# by proundmega
...
def usage():
    ...
    if you want to create a game data package, use:

    	python pkg.py --contentid my_game.pkg --gamedata
...
def main():
	...
	gamedata = False
	try:
		opts, args = getopt.getopt(sys.argv[1:], "hx:dvl:c:", ["help", "extract=", "version", "list=", "contentid=", "gamedata"])
	...
	for opt, arg in opts:
		if opt in ("-h", "--help"):
			...
			contentid = arg
		elif arg in ("--gamedata") or opt in ("--gamedata"):
			gamedata = True
		else:
			usage()
			sys.exit(2)
	if extract:
		...
		if len(args) == 1 and contentid != None and gamedata == True:
			pack(args[0], contentid, None, 0x10)
